...
int dam_getCaracter(char* pResultado, char* mensaje,char* mensajeError, int reintentos)
{
	int retorno = -1;
	char bufferChar[2];
...
static int getCharacter(char* pResultado)
{
    ...
    if(pResultado != NULL && !myGets(bufferString,sizeof(bufferString)) && esCaracter(bufferString,sizeof(bufferString)))
    {
        retorno=0;
...
static int esCaracter(char* cadena, int limite)
{
    ...
    {
        retorno=1;
        if((cadena[0]<'A' || cadena[0]>'Z') && (cadena[0]<'a' || cadena[0]>'z') && limite>2)
        {
            retorno=0;
        }
    }
